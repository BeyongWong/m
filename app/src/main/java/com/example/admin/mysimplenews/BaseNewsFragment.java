...
public abstract class BaseNewsFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
    ...
    public View initView(LayoutInflater inflater, @Nullable ViewGroup container) {
        ...
        this.recycler = (RecyclerView) rootiew.findViewById(R.id.recycler);
...
// 将网络请求的方法 抽象给父类
    ...
    /**
     * @param con
     * @param type 0表示加载的是第一页数据，1就是上啦加载
     */
    public abstract void loadSuccess(String con, int type);
    // 进行json解析 对list根据type进行操作
        ...
        public void onBindViewHolder(MyViewHolder holder, int position) {
            if (position < list.size()) {
               NewsItemBean newsItemBean = list.get(position);
                ...
                Glide.with(getActivity()).load(newsItemBean.imagePath).placeholder(R.drawable.jianbina).into(iv);
            }
        }
