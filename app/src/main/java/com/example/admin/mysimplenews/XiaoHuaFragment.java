...
public class XiaoHuaFragment extends BaseNewsFragment {
    @Override
    public void initData() {
        super.initData();
        String s = SPUtils.get(getClass().getName());
        if(TextUtils.isEmpty(s)) {
            loadData("http://c.m.163.com/nc/article/list/T1350383429665/0-20.html", 0);
        }else{
            loadSuccess(s,0);
        }
    }
    ...
    private void addData(String con) {
        try {
            JSONArray arr = new JSONObject(con).getJSONArray("T1350383429665");
            Log.e("tag","shuju--->"+arr.length());
               if(arr.length()>2){
                   setNormalState();
...
//                    if(!TextUtils.isEmpty(digest))  可以判断
    ...
    public void loadMore() {
        loadData("http://c.m.163.com/nc/article/list/T1350383429665/"+((page+1)*20)+"-20.html",1);
